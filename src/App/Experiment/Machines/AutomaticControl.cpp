...
//                double heavyLoadFall = (settings["heavy_load"].toDouble() / settings["leak_period"].toInt()) * m_continuousLeakPeriod;
    ...
    void AutomaticControl::pumpingBarrelLeakDetect()
    {
        ...
        m_pumpingLeakDetection.setParams(m_currentBarrel, chamber["pumping_time"].toInt(), chamber["leak_max"].toDouble(), ((chamber["barrel_delay"].toInt() / 1000) * samplesPerSecond), chamber["barrel_delay"].toInt());

...
namespace App { namespace Experiment { namespace Machines { namespace Functions
{
    ...
    void EmergancyStopFunctions::isPressed()
    {
        // Generate command
        QVariantMap command = m_commandConstructor.emergancyStopIsPressed();
    ...
    void EmergancyStopFunctions::validateIsPressed()
    {
        ...
        if(command != nullptr && command->package.value("cast_status").toBool())
        {
            ...
            if(package.value("status").toBool() == true)
            {
                ...
                emit emit_pressed(successPackage);
                return;
            }
            else
            {
                // Emit safe to proceed
                emit emit_notpressed(successPackage);
                return;
            }
        }
        else
